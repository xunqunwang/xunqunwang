...
func TestProxy(t *testing.T) {
	engine := bm.Default()
	engine.GET("/icon", NewAlways("http://api.xunqunwang.com/x/web-interface/index/icon"))
	engine.POST("/x/web-interface/archive/like", NewAlways("http://api.xunqunwang.com"))
...
func TestZoneProxy(t *testing.T) {
	engine := bm.Default()
	engine.GET("/icon", NewZoneProxy("sh004", "http://api.xunqunwang.com/x/web-interface/index/icon"), func(ctx *bm.Context) {
		ctx.AbortWithStatus(500)
	})
	engine.GET("/icon2", NewZoneProxy("none", "http://api.xunqunwang.com/x/web-interface/index/icon2"), func(ctx *bm.Context) {
		ctx.AbortWithStatus(200)
	})
	ug := engine.Group("/update", NewZoneProxy("sh004", "http://api.xunqunwang.com"))
